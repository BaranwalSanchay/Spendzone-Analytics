...
Every modeling and dataset metric here (2a, 2b, 2c, 3c) runs against the **synthetic
sample data** in `scripts/sample_data/`, generated by `scripts/generate_sample_data.py`
-- not the original proprietary data, which was scrubbed during the Market-Lens ->
Spendzone rebrand. These results demonstrate that the pipeline runs correctly end to
end and produces internally consistent output on realistic-shaped data; they are
**not** evidence that the platform produced, or would produce, any real business value,
lift, or ROI. A number like "2.23% projected lift" or "6% of spend in negative-ROI
channels" describes what the code computes on invented data with invented noise -- it
would look different, possibly very different, on real spend and real revenue.

3a and 3b (ingestion throughput, API latency) measure real infrastructure behavior --
actual FastAPI/uvicorn code running actual CPU cycles -- so those numbers are real
regardless of the data's origin, though still specific to one machine and one
measurement run (see each section's own caveats).

4a (the LangGraph concurrency fix) is a code-behavior test, not a data claim -- it
holds regardless of what data the pipeline is fed.

4b (report runtime/cost) is a real run against the real Groq API, spending real money
-- the runtime and cost are real, but the *content* the pipeline generated is an
analysis of the synthetic sample data, not of any real business. That run also did not
complete cleanly: one of 7 sections failed on a Groq rate limit, and the reported token
total is a known undercount -- both stated plainly in 4b rather than smoothed over.

In short: this file shows the pipeline works, and roughly how fast, how expensive, and
how accurate it is on data shaped like the real thing. It does not show what the
platform would find, cost, or achieve on the original proprietary data or in a real
deployment.

## What's defensible for a resume bullet

I don't have your actual current bullet text (it isn't checked into this repo, and I
didn't want to invent placeholder bullets to "revise") -- paste them and I'll do a
precise rewrite against the numbers above. Until then, here's what the measurements in
this file do and don't support:

**Supported, with the numbers to use:**
- Architecture: FastAPI backend, React dashboard, LangGraph multi-agent report
  generator, bi-level budget optimizer -- true regardless of data.
- Ingestion throughput up to ~747K rows/sec on `/api/upload` (3a) -- state the file
  size it was measured at.
- API latency: `/api/upload` p50 71ms/p95 85ms, `/api/budget-scenario` p50 34ms/p95
  64ms, at concurrency 10 on one machine (3b) -- state the concurrency and that it's
  one machine, not a production deployment.
- Fixed a LangGraph concurrent-state-update bug affecting **4 of 7** report sections
  (4a) -- not 3, not 5.
- Generates a 7-section AI report via Groq/LangGraph in ~7.5 minutes of active work
  (excluding built-in rate-limit pauses) for under a cent in tracked LLM tokens (4b) --
  but see the next section; this specific run didn't fully succeed.

**Not supported -- drop or reframe:**
- Any "%X lift" or "$Y in negative-ROI spend" claim without "on synthetic sample data"
  attached, and ideally "upper bound" / "poorly identified regression" too (2a, 2c).
- Any "model beats baseline" or "validated forecasting accuracy" claim -- the 3-month
  backtest doesn't show that (2b).
- Any Robyn benchmark comparison -- never run in this environment (2d).
- Any "Nx faster than manual" or "saves N analyst-hours" claim -- no real baseline
  exists to compare against (4c).
- Any claim implying the report pipeline reliably completes all 7 sections without
  failure -- the one real run in this repo didn't (4b).
